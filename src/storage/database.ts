...
let _db: any = null;
const FALLBACK_DB_STORAGE_KEY = "ChatBot:browser-db-fallback:v1";

async function getDb() {
  if (_db) return _db;
  try {
    const { default: Database } = await import("@tauri-apps/plugin-sql");
    _db = await Database.load("sqlite:ChatBot.db");
  } catch {
...
function createInMemoryDb() {
  const tables: Record<string, Map<string, Record<string, any>>> = loadFallbackTables();

  function persistTables() {
    if (typeof localStorage === "undefined") return;
    try {
      const serialized = Object.fromEntries(
        Object.entries(tables).map(([tableName, table]) => [tableName, Array.from(table.values())]),
      );
      localStorage.setItem(FALLBACK_DB_STORAGE_KEY, JSON.stringify(serialized));
    } catch (error) {
      console.warn("[DB] Failed to persist browser fallback DB:", error);
    }
  }
...
function loadFallbackTables() {
  const emptyTables: Record<string, Map<string, Record<string, any>>> = {
    conversations: new Map(),
    messages: new Map(),
    message_blocks: new Map(),
  };

  if (typeof localStorage === "undefined") return emptyTables;

  try {
    const raw = localStorage.getItem(FALLBACK_DB_STORAGE_KEY);
    ...
    for (const tableName of Object.keys(emptyTables)) {
      ...
      emptyTables[tableName] = new Map(
        rows.map((row) => [String(row.id ?? `${tableName}-${Math.random()}`), row]),
      );
    }
  } catch (error) {
    console.warn("[DB] Failed to restore browser fallback DB:", error);
  }
