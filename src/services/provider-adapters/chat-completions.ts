...
import { probeReasoningCapability } from "./reasoning-probe";
import { buildChatCompletionsReasoningPayload } from "./chat-completions-reasoning";
...
function buildRequestBody(
  ...
  return {
    ...
    ...(identity?.params?.temperature !== undefined
      ...
      : {}),
    ...buildChatCompletionsReasoningPayload(baseUrl, reasoningEffort),
...
export class ChatCompletionsAdapter implements ProviderAdapter {
  ...
  async probeCapabilities(params: ProbeParams): Promise<ProbeResult> {
    ...
    // Probe reasoning
    caps.reasoning = await probeReasoningCapability({
      ...
      body: {
        ...
        max_tokens: 1,
        messages: [{ role: "user", content: "hi" }],
        ...buildChatCompletionsReasoningPayload(params.baseUrl, "medium"),
      },
    });
