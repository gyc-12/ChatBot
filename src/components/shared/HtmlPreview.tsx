...
function formatHtmlForDisplay(source: string) {
  ...
  return lines
    .map((rawLine) => {
      ...
      const opensAndClosesOnSameLine =
        /^<([a-zA-Z0-9:-]+)(\s[^>]*)?>.*<\/\1>$/.test(line) && !line.includes("\n");
      ...
      if (!isClosingTag && !isSelfClosing && !opensAndClosesOnSameLine && /^</.test(line)) {
        indentLevel += 1;
      }
...
export const HtmlPreview = memo(function HtmlPreview({
  ...
  const displayCode = useMemo(() => {
    if (language === "html" || language === "svg") {
      return formatHtmlForDisplay(code);
    }
    return code;
  }, [code, language]);
      ...
      >
        ...
        {activeTab === "code" && (
          ...
          >
            {highlightedHtml ? (
              <div
                className="shiki-code text-[13px] leading-relaxed [&_code]:bg-transparent [&_pre]:m-0 [&_pre]:bg-transparent [&_pre]:px-3 [&_pre]:py-3"
                dangerouslySetInnerHTML={{ __html: highlightedHtml }}
              />
            ) : (
              <pre
                className="m-0 px-3 py-3 font-mono text-[13px] leading-relaxed"
